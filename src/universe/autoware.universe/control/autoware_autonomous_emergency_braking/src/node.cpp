  ...
  collision_data_keeper_(this->get_clock())
{
  ...
  // Service
  {
    aeb_activation_service_ = this->create_service<Trigger>(
      "~/activate_aeb", std::bind(&AEB::onAEBActivation, this, std::placeholders::_1, std::placeholders::_2));
  }
  // Diagnostics
  {
  ...
  // AEB activation control parameters
  aeb_activation_duration_ = declare_parameter<double>("aeb_activation_duration", 5.0);
  ...
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
  ...
  // Activate AEB
  aeb_activated_ = true;
  aeb_activation_time_ = this->get_clock()->now();
  
  RCLCPP_INFO(get_logger(), "[AEB] AEB activated on demand for %.1f seconds", aeb_activation_duration_);
  
  response->success = true;
  response->message = "AEB activated successfully";
}
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  updateParam<double>(parameters, "aeb_activation_duration", aeb_activation_duration_);
...
void AEB::onCheckCollision(DiagnosticStatusWrapper & stat)
{
  ...
  RCLCPP_DEBUG(get_logger(), "[AEB] onCheckCollision: collision_detected=%s", collision_detected ? "true" : "false");
  RCLCPP_DEBUG(get_logger(), "[AEB] onCheckCollision: collision_data_keeper_.checkCollisionExpired()=%s", 
                collision_data_keeper_.checkCollisionExpired() ? "true" : "false");
  ...
  if (!collision_data_keeper_.checkCollisionExpired()) {
    ...
    RCLCPP_INFO(get_logger(), "[AEB] Diagnostic system reporting emergency brake");
  } else {
    const std::string error_msg = "[AEB]: No Collision";
    const auto diag_level = DiagnosticStatus::OK;
    stat.summary(diag_level, error_msg);
    RCLCPP_DEBUG(get_logger(), "[AEB] Diagnostic system reporting no collision");
  }
...
bool AEB::checkCollision(MarkerArray & debug_markers)
{
  ...
  // step1.5. check if AEB is manually activated or if activation time has expired
  RCLCPP_DEBUG(get_logger(), "[AEB] Checking manual activation: aeb_activated_=%s", aeb_activated_ ? "true" : "false");
  
  if (aeb_activated_) {
    const auto now = this->get_clock()->now();
    const auto elapsed_time = (now - aeb_activation_time_).nanoseconds() * 1e-9;
    
    RCLCPP_DEBUG(get_logger(), "[AEB] Manual activation elapsed time: %.3f seconds", elapsed_time);
    
    if (elapsed_time > aeb_activation_duration_) {
      aeb_activated_ = false;
      RCLCPP_INFO(get_logger(), "[AEB] Manual AEB activation expired after %.1f seconds", elapsed_time);
    } else {
      // Force collision detection when manually activated
      RCLCPP_INFO(get_logger(), "[AEB] Manual AEB activation active, forcing collision check");
      // Create a dummy collision to trigger emergency braking
      // We need to refresh this every time to keep it from expiring
      ObjectData dummy_collision;
      dummy_collision.stamp = now;  // Use current time to keep it fresh
      dummy_collision.position = geometry_msgs::msg::Point();
      dummy_collision.velocity = 0.0;
      dummy_collision.rss = 1.0;
      dummy_collision.distance_to_object = 0.1;
      collision_data_keeper_.setCollisionData(dummy_collision);
      RCLCPP_INFO(get_logger(), "[AEB] Dummy collision data refreshed, returning true");
      return true;
    }
  }
